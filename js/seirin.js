...
if (herder.indexOf("7.webp") > 0) {
    var page_img = getRand(1, 8);
    $('#page-header').attr('style','background-image: url(/img/pic/'+page_img+'.webp)')
    $('#footer').attr('style','background-image: url(/img/pic/'+page_img+'.webp)')
}
// $('.prev-cover').attr('data-original', 'https://cdn.jsdelivr.net/gh/AyagawaSeirin/Blog@gh-pages/img/headimg/' + getRand(1, 178) + '.webp');
// $('.prev-cover').attr('data-lazy-src', '/img/headimg/' + getRand(1, 178) + '.webp');
$('.prev-cover').attr('src', '/img/headimg/' + getRand(1, 178) + '.webp');
// $('.next-cover').attr('data-original', 'https://cdn.jsdelivr.net/gh/AyagawaSeirin/Blog@gh-pages/img/headimg/' + getRand(1, 178) + '.webp');
// $('.next-cover').attr('data-lazy-src', '/img/headimg/' + getRand(1, 178) + '.webp');
$('.next-cover').attr('src', '/img/headimg/' + getRand(1, 178) + '.webp');
...
for (j = 0; j < relatedPosts_cover.length; j++) {
    ...
    // relatedPosts_cover[j].setAttribute('data-lazy-src','/img/headimg/' + getRand(1, 178) + '.webp');
    relatedPosts_cover[j].setAttribute('src','/img/headimg/' + getRand(1, 178) + '.webp');
}
$('.comment-headline').html('<i class="fas fa-comments fa-fw"></i><span> 评论</span>')
...
if (!browser.versions.mobile) {
    ...
    L2Dwidget.init({pluginRootPath: "/live2dw/", pluginJsPath: "lib/", pluginModelPath: "22/", model: {jsonPath: "/live2dw/22/model.json"}, display: {superSample: 2, width: 220, height: 330, position: "right", hOffset: 0, vOffset: -20}, log: !1, tagMode: !1})
}
